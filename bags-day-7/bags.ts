const input_test = `light red bags contain 1 bright white bag, 2 muted yellow bags.
...
type BagInfo = { amount: number; type: string };

const rules_map = input
  .split(`\n`)
  .reduce((rules: Map<string, BagInfo[]>, line) => {
    const [container, content] = line.split(`contain`).filter(Boolean);
    const contents = content
      .split(/ bags?[\.,]/g)
      .map((x) => {
        const trimmed = x.trim();
        if (!trimmed || trimmed === `no other`) return null;
        const [amount, ...type] = trimmed.split(` `);
        return { amount: Number(amount), type: type.join(` `) }
      })
      .filter(x => x !== null) as BagInfo[];
    return new Map([...rules, [container.replace(` bags `, ``), contents]]);
  }, new Map<string, BagInfo[]>());

console.log(rules_map);
//const counts = new Set<string>();
//
//for (const bag_type of rules_map.keys()) {
//  const stack = [bag_type];
//  if (counts.has(bag_type)) continue;
//  while (stack.length > 0) {
//    const type = stack.pop()!;
//
//    if (type.includes("shiny gold")) {
//      counts.add(bag_type)
//      break;
//    } else {
//      stack.push(...(rules_map.get(type) || []).map(x => x.type));
//    }
//  }
//}

//counts.delete("shiny gold");
//console.log(counts, counts.size);

const calc_count = (next: string = 'shiny gold', mem: Map<string, number> = new Map): number => {
  const m = mem.get(next)
  if (m) return m

  const contents = rules_map.get(next) || [];
  const count = contents.reduce((sum, { amount, type }) => {
    const count_for_type = calc_count(type, mem) + 1
    return amount * count_for_type + sum
  }, 0)
