drop table if exists raw_passports;
...
select
    passport_id,
    max(case when type = 'eyr' then value::int else null end) as eyr,
    max(case when type = 'pid' then value else null end) as pid,
    max(case when type = 'hgt' then value else null end) as hgt,
    max(case when type = 'cid' then value else null end) as cid,
    max(case when type = 'iyr' then value::int else null end) as iyr,
    max(case when type = 'byr' then value::int else null end) as byr,
    max(case when type = 'hcl' then value else null end) as hcl,
    max(case when type = 'ecl' then value else null end) as ecl
into
    passports
from
    (
        select
            split_part(unnest(tokens), ':', 1) as type,
            split_part(unnest(tokens), ':', 2) as value,
            row_number() over () as passport_id
        from
            (
                select
                    regexp_split_to_array(passport, E'[ \n]')
                from
                    raw_passports as passport_fields
            ) as dt(tokens)
    ) as kekw
group by
    passport_id;
...
where
    ...
    (
        array_length(regexp_match(hgt, E'^(1[5-8][0-9]|19[0-3])cm$'), 1) = 1
            or
        array_length(regexp_match(hgt, E'^(59|6[0-9]|7[0-6])in$'), 1) = 1
    ) and
    array_length(regexp_match(hcl, E'^#[a-f0-9]{6}$'), 1) = 1 and
    array['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'] @> array[ecl] and
    array_length(regexp_match(pid, E'^[0-9]{9}$'), 1) = 1;
