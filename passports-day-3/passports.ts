...
const answer = input
  .split(`\n\n`)
  .reduce(
    (valid_count, passport_info) => {
      const keys = new Set(passport_info.match(/(\w+):/g).map(x => x.slice(0, -1)));
      const has_all = `byr iyr eyr hgt hcl ecl pid`.split(` `).every(key => keys.has(key));
      return valid_count + Number(has_all);
    },
    0
  );
