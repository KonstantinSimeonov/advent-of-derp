...
const simulate = (input: string) => {
  ...
  const delta_count = deltas.length
  const neighbs = (x: number, y: number) => {
    const ns: Record<'x' | 'y', number>[] = []
    for (let i = 0; i < delta_count; ++i) {
      const [dx, dy] = deltas[i]

      for (
        let nx = x + dx, ny = y + dy;
        0 <= ny && ny < row_count && 0 <= nx && nx < col_count;
        nx += dx, ny += dy
      ) {
        if (rows[ny][nx] === `L`) {
          ns.push({ x: nx, y: ny })
  ...
  const seats = rows.map(
    (row: string, y: number): Array<{ occupants: 0 | 1; neighbs: Record<'x' | 'y', number>[]; } | null> =>
      Array.prototype.map.call(
        row,
        (_, x) => rows[y][x] === `L` ? ({ occupants: 0, neighbs: neighbs(x, y) }) : null
      ) as any
  )

  let converged = false
  do {
    const updates: { x: number; y: number, o: 1 | 0 }[] = []
    for (let y = 0; y < row_count; ++y)
      for (let x = 0; x < col_count; ++x) {
        if (!seats[y][x]) continue

        let takenCount = 0
        const { occupants, neighbs } = seats[y][x]!
        for (const { x, y } of neighbs)
          takenCount += seats[y][x]!.occupants

        if (takenCount === 0 && occupants === 0)
          updates.push({ y, x, o: 1 })
        else if (takenCount > 4 && occupants === 1)
          updates.push({ y, x, o: 0 })
      }

    for (const { x, y, o } of updates)
      seats[y][x]!.occupants = o

    converged = updates.length === 0
  } while (!converged)

  return seats.reduce(
    (sum, row) => sum + row.reduce((row_sum, s) => row_sum + (s ? s.occupants : 0), 0),
    0
  )
}
