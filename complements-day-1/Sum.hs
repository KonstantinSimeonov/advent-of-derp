...
nums :: Array Int Int
nums = listArray (0, length input - 1) input

lowerBound :: Array Int Int -> Int -> Int -> Int -> Int
lowerBound xs bound left right
  | left >= right = left
  | xs ! mid > bound = lowerBound xs bound left mid
  | xs ! mid < bound = lowerBound xs bound (mid + 1) right
  | otherwise = mid

  where mid = (left + right) `div` 2

find3Complements :: Array Int Int -> Int -> Int
find3Complements xs sum = find2 $ snd $ bounds xs

  where find2 :: Int -> Int
        find2 right
          = let maxCompl = sum - (xs ! right)
                maxBound = lowerBound xs maxCompl 0 right
                result
                  = fst $
                      List.foldl'
                        (\ (res, compls) i ->
                           case res of
                               Nothing -> let newCompls
                                                = IntSet.insert
                                                    (maxCompl - (xs ! i))
                                                    compls
                                            in
                                            if IntSet.member (xs ! i) newCompls
                                              then
                                              (Just $
                                                 (xs ! right, xs ! i,
                                                  maxCompl - (xs ! i)),
              ...
              in
              case result of
                  Nothing -> find2 $ right - 1
                  Just (a, b, c) -> a * b * c

main = print $ find3Complements nums 2020
